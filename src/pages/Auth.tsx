...
const Auth = () => {
  ...
  const handlePostAuthRedirect = async () => {
    ...
    let userType = session?.user?.user_metadata?.user_type as string | undefined;
    // Source de vérité = table profiles (les metadata peuvent être absentes ou "both")
    if (session?.user && (!userType || userType === 'both')) {
  ...
  const loginSchema = z.object({
    ...
    password: z.string().min(6, "Mot de passe requis (6+ caractères)").max(72),
  });
  ...
  const resolveDashboardPath = async (fallback: UserType): Promise<string> => {
    const { data: { session } } = await supabase.auth.getSession();
    let userType: string | undefined = session?.user?.user_metadata?.user_type as string | undefined;
    if (session?.user && (!userType || userType === 'both')) {
      const { data: profile } = await supabase
        .from('profiles')
        .select('user_type')
    ...
    const effective = (userType === 'walker' || userType === 'owner') ? userType : fallback;
    return effective === 'walker' ? '/walker/dashboard' : '/dashboard';
  };
  ...
  const handleForgotPassword = async () => {
    const emailInput = (document.getElementById('login-email') as HTMLInputElement | null)?.value?.trim();
    const parsed = z.string().email().safeParse(emailInput);
    if (!parsed.success) {
      toast({ title: "Email requis", description: "Renseignez votre email puis cliquez à nouveau sur 'Mot de passe oublié'.", variant: "destructive" });
      return;
    }
  ...
  const benefits = [
    { icon: Shield, text: "Profils et services renseignés" },
    { icon: CheckCircle, text: "Demandes suivies dans votre espace" },
    { icon: Star, text: "Informations de mission selon le service" },
  ];
  ...
  const roleCards = [
    {
      ...
      description: "Trouvez un Accompagnateur pour organiser les besoins de votre compagnon",
      features: [
        { icon: Calendar, text: "Réservez des promenades" },
        { icon: Shield, text: "Services renseignés" },
        ...
        <div className="hidden lg:flex lg:w-1/2 relative overflow-hidden">
          ...
          <div className="absolute inset-0 flex flex-col justify-center px-12">
            <motion.div initial={{ opacity: 0, x: -30 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8 }}>
              <Badge className="mb-4 bg-white/20 text-white border-white/30">DogWalking</Badge>
              <h1 className="text-4xl lg:text-5xl font-bold text-white mb-4">Rejoignez la communauté DogWalking</h1>
              <p className="text-lg text-white/80 mb-8 max-w-md">
                Organisez vos demandes et vos informations de mission avec les Accompagnateurs inscrits sur DogWalking.
              </p>
