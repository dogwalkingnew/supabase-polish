...
const WalkerDashboardV3 = () => {
  ...
  return (
    <div className="min-h-dvh bg-warm/50 text-foreground">
      <Header />
      <main className="container w-full max-w-7xl py-24">
        <div className="dashboard-gradient relative mb-8 overflow-hidden rounded-3xl p-6 text-white shadow-elevated md:p-8 dashboard-interactive">
        ...
        {loading ? <div className="dashboard-glass flex min-h-48 items-center justify-center rounded-2xl"><Loader2 className="h-6 w-6 animate-spin text-primary" /></div> : (
          <div className="space-y-6">
            <Card className={`dashboard-glass dashboard-interactive ${verified ? "border-primary/30 bg-primary/5" : "border-amber-400/40 bg-amber-50"}`}><CardContent className="flex flex-col gap-3 p-5 sm:flex-row sm:items-center sm:justify-between"><div className="flex items-start gap-3">{verified ? <CheckCircle2 className="mt-0.5 h-5 w-5 text-primary" /> : <ShieldAlert className="mt-0.5 h-5 w-5 text-amber-700" />}<div><p className="font-semibold">{verified ? "Profil opérationnel" : "Profil en attente de validation"}</p><p className="text-sm text-muted-foreground">{verified ? "Vous pouvez confirmer ou refuser les demandes qui vous sont directement attribuées." : "Vous ne pouvez pas répondre à une demande tant qu’une validation administrative n’est pas enregistrée."}</p></div></div><Button asChild size="sm" variant="outline"><Link to="/walker/register">Compléter mon dossier</Link></Button></CardContent></Card>
            <div className="grid gap-4 md:grid-cols-2"><Card className="dashboard-glass dashboard-interactive"><CardHeader className="pb-3"><CardDescription className="flex items-center gap-2"><ClipboardList className="h-4 w-4" /> Missions actives</CardDescription><CardTitle>{activeBookings.length}</CardTitle></CardHeader><CardContent className="text-sm text-muted-foreground">Confirmées ou en cours, sans calcul de revenu ni paiement intégré.</CardContent></Card><Card className="dashboard-glass dashboard-interactive"><CardHeader className="pb-3"><CardDescription className="flex items-center gap-2"><UserRound className="h-4 w-4" /> Disponibilités</CardDescription><CardTitle>À renseigner</CardTitle></CardHeader><CardContent className="text-sm text-muted-foreground">La gestion de disponibilités n’est pas encore active ; confirmez uniquement les créneaux que vous pouvez assurer.</CardContent></Card></div>
            <Card className="dashboard-glass dashboard-interactive"><CardHeader><CardTitle className="flex items-center gap-2"><CalendarDays className="h-5 w-5 text-primary" /> Réservations attribuées</CardTitle><CardDescription>Les coordonnées détaillées restent limitées aux participants de la mission.</CardDescription></CardHeader><CardContent>{bookings.length === 0 ? <div className="rounded-xl border border-dashed p-8 text-center text-muted-foreground"><Dog className="mx-auto mb-3 h-8 w-8 text-primary/60" />Aucune réservation ne vous est attribuée actuellement.</div> : <div className="space-y-3">{bookings.map((booking) => <div key={booking.id} className="dashboard-glass dashboard-interactive flex flex-col gap-3 rounded-xl p-4 sm:flex-row sm:items-center sm:justify-between"><div><div className="mb-1 flex flex-wrap items-center gap-2"><span className="font-semibold">{serviceLabel[booking.service_type || ""] || "Prestation"}</span><Badge variant="secondary">{statusLabel[booking.status || ""] || booking.status || "—"}</Badge></div><p className="text-sm text-muted-foreground">{booking.dogs?.name ? `Animal : ${booking.dogs.name} · ` : ""}{booking.scheduled_date ? new Date(booking.scheduled_date).toLocaleDateString("fr-FR") : "Date à confirmer"}{booking.scheduled_time ? ` à ${booking.scheduled_time.slice(0, 5)}` : ""}{booking.duration_minutes ? ` · ${booking.duration_minutes} min` : ""}</p>{booking.address && <p className="mt-1 flex items-center gap-1 text-xs text-muted-foreground"><MapPin className="h-3.5 w-3.5" /> Zone renseignée pour la mission</p>}</div><div className="flex flex-wrap gap-2"><Button asChild size="sm" variant="outline"><Link to={`/bookings/${booking.id}`}>Voir la mission</Link></Button>{booking.status === "pending" && verified && <><Button size="sm" disabled={respondingId === booking.id} aria-busy={respondingId === booking.id} onClick={() => void respondToBooking(booking.id, "accept")}>{respondingId === booking.id ? <Loader2 className="h-4 w-4 animate-spin" aria-label="Enregistrement" /> : "Accepter"}</Button><Button size="sm" variant="destructive" disabled={respondingId === booking.id} aria-busy={respondingId === booking.id} onClick={() => void respondToBooking(booking.id, "decline")}>{respondingId === booking.id ? <Loader2 className="h-4 w-4 animate-spin" aria-label="Enregistrement" /> : "Refuser"}</Button></>}</div></div>)}</div>}</CardContent></Card>
          </div>
        )}
      </main>
      <Footer />
