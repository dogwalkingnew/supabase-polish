...
const OwnerHomeImproved = ({ pets, nextMission, onReserve, onViewAllPets, onViewAllMissions, onViewMissionDetails }: OwnerHomeImprovedProps) => (
  <div className="min-h-dvh bg-warm/70 pb-28">
    ...
    <main className="mx-auto -mt-3 w-full max-w-7xl space-y-6 px-4 sm:px-6 lg:px-10">
      ...
      <section>
        ...
        {pets.length === 0 ? (
          <div className="dashboard-glass dashboard-interactive rounded-2xl p-5 text-sm text-muted-foreground">
            Ajoutez un animal avant de formuler une demande ; chaque mission reste rattachée au profil concerné.
          </div>
        ) : (
          <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
            {pets.slice(0, 2).map((pet) => (
              <article key={pet.id} className="dashboard-glass dashboard-interactive overflow-hidden rounded-2xl p-2.5">
                <img src={pet.photo} alt={pet.name} className="h-24 w-full rounded-xl object-cover" loading="lazy" />
                <div className="px-1 pt-2">
                  <h2 className="truncate font-bold text-foreground">{pet.name}</h2>
      ...
      {nextMission && (
        <section>
          ...
          <article className="dashboard-glass dashboard-interactive rounded-2xl p-4">
            <div className="flex gap-3">
              {nextMission.walkerPhoto ? (
                ...
                <div className="icon-bg-primary flex h-14 w-14 shrink-0 items-center justify-center rounded-full text-lg font-bold text-primary" aria-hidden="true">
              ...
              <div className="min-w-0 flex-1">
                ...
                <div className="mt-3 space-y-1.5 text-xs text-muted-foreground">
                  <p className="flex items-center gap-2"><CalendarDays className="h-3.5 w-3.5 text-primary" />{nextMission.date} · {nextMission.time || "heure à confirmer"}</p>
                  {nextMission.address && <p className="flex items-center gap-2"><MapPin className="h-3.5 w-3.5 text-primary" />{nextMission.address}{nextMission.city ? `, ${nextMission.city}` : ""}</p>}
            ...
            <button type="button" onClick={onViewMissionDetails} aria-label={`Suivre la demande de ${nextMission.walkerName}`} className="mt-4 flex w-full items-center justify-center gap-2 rounded-xl border border-primary/30 px-3 py-2.5 text-sm font-bold text-primary transition-colors hover:bg-primary/5">
