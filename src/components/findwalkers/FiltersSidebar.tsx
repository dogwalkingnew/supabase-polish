/** DogWalking — Confiance canine de proximité : filtres chaleureux, vert forêt pour toute sélection publique. */
...
const FiltersSidebar = () => {
      ...
      <button onClick={() => toggle(id)} className="flex items-center justify-between w-full text-sm font-bold text-foreground">
        <span className="flex items-center gap-2">
          ...
          {count !== undefined && count > 0 && (
            <span className="w-5 h-5 rounded-full text-[10px] font-bold flex items-center justify-center text-primary-foreground bg-primary">{count}</span>
          )}
        </span>
  ...
  return (
    <div className="bg-card/95 rounded-[1.25rem] border border-primary/15 p-5 shadow-soft">
      <div className="flex items-center justify-between mb-4">
        ...
        {hasActiveFilters && (
          <button onClick={resetFilters} className="flex items-center gap-1 text-xs font-semibold text-primary hover:underline">
          ...
          {distances.map((d) => (
            <button key={d.value} onClick={() => setDistance(d.value)}
              className={cn("px-3 py-1.5 rounded-lg text-xs font-bold border transition-all",
                distance === d.value ? "border-primary text-primary-foreground bg-primary" : "border-border text-foreground/70 hover:border-primary/35"
              )}>{d.label}</button>
          ))}
        </div>
      </Section>
      ...
      <button className="w-full mt-4 font-bold py-2.5 rounded-xl transition-all text-sm text-primary-foreground bg-primary shadow-button hover:shadow-lg hover:bg-primary/90">
