...
import BookingApplicationsList from "@/components/booking/BookingApplicationsList";
  ...
  const handleConfirm = async (bookingId: string) => {
    ...
    try {
      ...
      if (booking) {
        ...
        const senderName = role === "walker" ? "L'Accompagnateur" : "Le Propriétaire";
        if (targetId) {
                ...
                <button className="w-full p-4 text-left" onClick={() => setExpandedId(isExpanded ? null : b.id)}>
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        <span className="font-bold text-sm text-foreground">🐕 {b.dogs?.name || b.dogName || "Animal"}</span>
                  ...
                  {isExpanded && (
                    <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }}
                      ...
                      <div className="px-4 pb-4 space-y-2">
                        ...
                        {role === "owner" && !b.walker_id && b.status === "pending" && (
                          <BookingApplicationsList bookingId={b.id} ownerId={b.owner_id} />
                        )}

                        {role === "walker" && !isDemo && b.walker_id && (b.status === "confirmed" || b.status === "in_progress") && (
                          <div className="pt-1">
                            <MissionStartButton
                              bookingId={b.id}
                              walkerId={b.walker_id}
                              dogName={b.dogs?.name || "Animal"}
                              ownerName={b.profiles?.full_name || "Propriétaire"}
                        ...
                        {role === "walker" && !isDemo && b.status === "in_progress" && (
                          <div className="pt-1">
                            <ValidateCodeInput
                              bookingId={b.id}
                              alreadyValidated={!!b.code_validated_at}
                            />
                          </div>
                        )}
