...
const WalkerEarningsTab = () => {
    ...
    try {
      ...
      if (earningsData && earningsData.length > 0) {
        ...
        const thisMonthEarnings = earningsData
          .filter(e => new Date(e.created_at ?? 0) >= startOfMonth)
          .reduce((sum, e) => sum + Number(e.net_amount), 0);

        const lastMonthEarnings = earningsData
          .filter(e => {
            const date = new Date(e.created_at ?? 0);
  ...
  const fetchFromBookings = async (userId: string) => {
    ...
    const commission = 0.13; // 13% commission v5.0 (87% reversés)
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground">
                        {new Date(t.created_at ?? 0).toLocaleDateString('fr-FR', {
                          day: 'numeric',
                          month: 'long',
                          year: 'numeric'
                        })}
        ...
        <CardContent className="flex items-start gap-4 pt-6">
          ...
          <div>
            ...
            <ul className="text-sm text-muted-foreground space-y-1">
              ...
              <li>• La commission DogWalking est de <strong>13%</strong> (vous percevez <strong>87%</strong>)</li>
