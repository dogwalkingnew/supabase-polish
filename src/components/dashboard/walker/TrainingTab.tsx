...
import { CheckCircle, BookOpen, Play, Award, ChevronRight, RotateCcw, Loader2 } from "lucide-react";
...
const STORAGE_KEY = "training_completed_modules";

// Persist completed modules: Supabase (authenticated) + localStorage (fallback/demo)
const useTrainingProgress = () => {
      ...
      if (user) {
        try {
          const { data, error } = await (supabase as any)
            .from("walker_training_progress")
            .select("module_id")
            .eq("user_id", user.id)
            .eq("completed", true);
          if (!error && data) {
            const ids = (data as any[]).map((r) => r.module_id);
  ...
  const markCompleted = async (moduleId: string, score: number) => {
  ...
  return { completedModules, loadingProgress, markCompleted };
};

const WalkerTrainingTab = () => {
  const { completedModules, loadingProgress, markCompleted } = useTrainingProgress();
  ...
  const progress = DEFAULT_TRAINING_MODULES.length > 0
    ? (completedModules.length / DEFAULT_TRAINING_MODULES.length) * 100
    : 0;
  ...
  const selectAnswer = async (answerIndex: number) => {
    ...
    if (newAnswers.length >= questions.length) {
      ...
      if (result.passed && !completedModules.includes(activeModule!.id)) {
        await markCompleted(activeModule!.id, result.score);
        toast({ title: "🎉 Module validé !", description: `Score : ${result.score}% — progression sauvegardée` });
      }
    } else {
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="p-4 space-y-6">
          <Card className="overflow-hidden">
            <CardContent className="p-6 text-center space-y-4">
              ...
              {passed && (
                <p className="text-xs text-green-600 font-semibold">✅ Progression sauvegardée</p>
              )}
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="p-4 space-y-4 pb-24">
      ...
      <div className="space-y-3">
        {DEFAULT_TRAINING_MODULES.map((module, idx) => {
          ...
          return (
            <motion.div key={module.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: idx * 0.08 }}>
